...
(defun ruby-electric-return-can-be-expanded-p()
  (interactive)
  (if (and ruby-electric-return
           (ruby-electric-code-at-point-p))
      (let* ((ruby-electric-keywords-re
              (concat ruby-electric-simple-keywords-re "\n"))
             (ruby-electric-expandable-do-re "do\n"))
        (save-excursion
          (let ((end (point)))
            (backward-word 1)
            (and (or (looking-at ruby-electric-expandable-do-re)
                     (looking-at ruby-electric-keywords-re))
                 (= (match-end 0) end)))))))


(defcustom ruby-electric-return t
  "*Controls whether pressing return should insert a closing 'end'
after appropriate keywords."
  :type 'boolean :group 'ruby-electric)
