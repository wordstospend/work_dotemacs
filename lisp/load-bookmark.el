;; Improves bookmarks, which are pretty nifty. Adds tags to bookmarking, bookmarks with the same name. 
;; Bookmarks also keep metadata, such as the number of times visited, and the time of the last visit
(require 'bookmark+)

;; This setup allows me to export the bookmarks to zsh, and along with a zsh setup that has cdablevars(setopt cdablevars), you can cd to the emacs bookmarks. 
